...
const handleGistSearch = (gists, search, listFunction) => {
  const results = gists.filter(({ id, description }) =>
    textMatchSearchWords(`${id} ${description}`, search.split(" "))
  )
  listFunction(results)
  // switch (type) {
  //   case "open":
  //     interactiveOpenGist(results)
  //     break
  //   case "copy":
  //     interactiveCopyGistId(results)
  //     break
  //   case "download":
  //     interactiveDownloadGist(results)
  //     break
